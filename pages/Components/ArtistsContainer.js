...
export default function ArtistsContainer() {
  ...
  if (artists) {
    let ArtistComponents = artists.map((artist, index) => {
      console.log(artist)
      return (<ArtistComponent key = {index} index = {index} artistImageSrc={artist.images[1].url} width = {artist.images[1].width} height = {artist.images[1].height} artistName = {artist.name} artistId = {artist.id} />)
    })
